\begin{tad}{\tadNombre{Sistema}}
	...
	\tadAxiomas [\paratodo{mapa}{\ m}, \paratodo{sistema}{\ s}, \paratodo{jugador}{\ j, j'}, \paratodo{posicion}{\ p, p'}, \paratodo{pokemon}{\ pk}, \paratodo{nat}{\ n}, \\ \paratodo{conj(jugador)}{\ js}, \paratodo{conj(posicion)}{\ ps}, \paratodo{multiconj(pokemon)}{\ pks}, \paratodo{conj(multiconj(pokemon))}{\ cpks}]
		...
		\tadAxioma{posCaptura(j, ps, s)}{juntarPosCaptura(j, posiciones(mapa(s)), s)}
		...
		\tadAxioma{pokemonACapturar(j, s)}{pokemonEnPos(posCaptura(j, posiciones(mapa(s)), s), s)}
		...
		\tadAxioma{capturaPokemon?(j, j', p', s)}{
			...
			$\neg\emptyset$?(jugadoresCapturandoPk(posCaptura(j, posiciones(mapa(s)), s)) - \{j'\}) $\yluego$ $\\$
			j = dameUno(jugadoresCapturandoPk(posCaptura(j, posiciones(mapa(s)), s)) - \{j'\})
		}
		...
		\tadAxioma{hayPokemon?(p, moverJugador(s,j,p'))}{hayPokemon?(p,s) $\yluego$ \\ $\neg$alguienCapturaPokemon?(jugadoresConectados(jugadores(s), s), p, j, p', s)}
		...
		{$\neg$vacio?(js) $\yluego$ (capturaPokemon?(dameUno(js), p, s) $\lor$ capturaPokemon?(sinUno(js), p, s))}
