\begin{tad}{\tadNombre{Sistema}}
	...
	\tadObservadores
		...
		\tadOperacion{movsLejosDePos}{jugador/j, posicion/p, sistema/s}{nat}{j $\in$ jugadoresConectados(jugadores(s)) \\ $\land$ p $\in$ posiciones(mapa(s))}
	...
	\tadOtrasOperaciones
		...
		\tadOperacion{alguienCapturaPokemon?}{conj(jugador)/js, posicion/p, sistema/s}{bool}{js $\subseteq$ jugadoresConectados(jugadores(s), s) $\land$ p $\in$ posiciones(mapa(s))}

		\tadOperacion{valorPokemones}{multiconj(pokemon)/pks, sistema/s}{nat}{0 $< \#$todosLosPokemons(s)}

		\tadOperacion{pokemonesADebitar}{jugador/j, nat/n, sistema/s}{multiconj(pokemon)}{j $\in$ jugadores(s) $\yluego$ n $\leq$ valorPokemones(pokemonsCapturados(j, s))}
