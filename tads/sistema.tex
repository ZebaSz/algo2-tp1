\begin{tad}{\tadNombre{Sistema}}
	...
	\tadOtrasOperaciones
		...
		\tadOperacion{capturandoPk?}{jugador/j, posicion/p, sistema/s}{bool}{j $\in$ jugadoresConectados(jugadores(s), s) $\land$ p $\in$ posiciones(mapa(s)))}
		\tadOperacion{capturando?}{jugador/j, conj(posicion)/ps, sistema/s}{bool}{j $\in$ jugadoresConectados(jugadores(s), s) $\land$ ps $\subseteq$ posiciones(mapa(s)))} 
		\tadOperacion{posCaptura}{jugador/j, sistema/s}{posicion}{j $\in$ jugadoresConectados(jugadores(s), s) $\yluego$ capturando?(j, posiciones(mapa(s)), s)} 
		\tadOperacion{juntarPosCaptura}{jugador/j, conj(posicion)/ps, sistema/s}{posicion}{j $\in$ jugadoresConectados(jugadores(s), s) $\land$ ps $\subseteq$ posiciones(mapa(s))) $\yluego$ capturando?(j, ps, s)} 
		\tadOperacion{jugadoresCapturandoPk}{posicion/p, sistema/s}{conj(jugador)}{p $\in$ posiciones(mapa(s)))}
		\tadOperacion{juntarJugadoresCapturando}{conj(jugador)/js, posicion/p, sistema/s}{conj(jugador)}{js $\subseteq$ jugadoresConectados(jugadores(s), s) $\land$ p $\in$ posiciones(mapa(s)))} 
		...
		% TODO ver si se puede simplificar esto
	...
	\tadAxiomas [\paratodo{mapa}{\ m}, \paratodo{sistema}{\ s}, \paratodo{jugador}{\ j, j'}, \paratodo{posicion}{\ p, p'}, \paratodo{pokemon}{\ pk}, \paratodo{nat}{\ n}, \\ \paratodo{conj(jugador)}{\ js}, \paratodo{conj(posicion)}{\ ps}, \paratodo{multiconj(pokemon)}{\ pks}, \paratodo{conj(multiconj(pokemon))}{\ cpks}]
		...
		\tadAxioma{capturandoPk?(j, p ,s)}{hayPokemon?(p, s) $\land$ distancia(posicionJugador(j, s), p) $\leq$ 2}

		\tadAxioma{capturando?(j, ps, s)}{\IF vacio?(ps) THEN false ELSE capturandoPk?(j, dameUno(ps), s) $\lor$ capturando(j, sinUno(ps), s) FI}

		\tadAxioma{posCaptura(j, s)}{juntarPosCaptura(j, posiciones(mapa(s)), s)}

		\tadAxioma{juntarPosCaptura(j, ps, s)}
		{\IF capturandoPk?(j, dameUno(ps), s) THEN 
			dameUno(ps) 
		ELSE
			juntarPosCaptura(j, sinUno(ps), s) 
		FI}


		\tadAxioma{pokemonACapturar(j, s)}{pokemonEnPos(posCaptura(j, s), s)}

		\tadAxioma{jugadoresCapturandoPk(p, s)}{juntarJugadoresCapturando(jugadoresConectados(jugadores(s), s), p, s)}
		
		\tadAxioma{juntarJugadoresCapturando(js, p, s)}
		{\IF vacio?(js) THEN 
			$\emptyset$ 
		ELSE
			{\IF capturandoPk?(dameUno(js), p, s) THEN
				Ag(dameUno(js), $\emptyset$)
			ELSE
				$\emptyset$
			FI} $\cup$ jugadoresCapturandoPk(sinUno(js), p, s)
		FI}

		\tadAxioma{capturaPokemon?(j, j', p', s)}{
			capturando?(j, posiciones(mapa(s)), s) $\yluego$ $\\ $
			movsLejosDePosDespuesDelMov(j, posCaptura(j, s), j', p', s) = 10 $\land $ \\
			$\neg\emptyset$?(jugadoresCapturandoPk(posCaptura(j, s), s) - \{j'\}) $\yluego$ $\\ $
			j = dameUno(jugadoresCapturandoPk(posCaptura(j, s), s) - \{j'\})
		}
