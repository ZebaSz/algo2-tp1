\begin{tad}{\tadNombre{Sistema}}
	...
	\tadAxiomas [\paratodo{mapa}{\ m}, \paratodo{sistema}{\ s}, \paratodo{jugador}{\ j, j'}, \paratodo{posicion}{\ p, p'}, \paratodo{pokemon}{\ pk}, \paratodo{nat}{\ n}, \\ \paratodo{conj(jugador)}{\ js}, \paratodo{conj(posicion)}{\ ps}, \paratodo{multiconj(pokemon)}{\ pks}, \paratodo{conj(multiconj(pokemon))}{\ cpks}]
		...
		{\IF conectado?(j) $\yluego$ capturaPokemon?(j, j', p', s) THEN
			Ag(pokemonACapturar(j, posiciones(mapa(s)), s)), $\emptyset$)
		ELSE
			$\emptyset$
		...
		\tadAxioma{capturando?(j, ps, s)}{vacio?(ps) $\oluego$ capturandoPK?(j, dameUno(ps), s) $\lor$ capturando(j, sinUno(ps), s)}

		\tadAxioma{posCaptura(j, ps, s)}{\IF vacio?(sinUno(ps)) THEN dameUno(ps) ELSE posCaptura(j, sinUno(ps), s) FI}

		\tadAxioma{pokemonACapturar(j, ps, s)}{pokemonEnPos(posCaptura(j, ps, s), s)}
		...
		\tadAxioma{capturaPokemon?(j, j', p', s)}{
			...
			2 $<$ distancia(posCaptura(j, posiciones(mapa(s)), s), p') $\land$
