\begin{tad}{\tadNombre{Sistema}}
	...
	\tadOtrasOperaciones
		...
		\tadOperacion{posCapturaAux}{jugador/j, conj(posicion)/ps, sistema/s}{posicion}{j $\in$ jugadoresConectados(jugadores(s), s) $\land$ ps $\subseteq$ posiciones(mapa(s))) $\yluego$ capturando?(j, ps, s)} 
	...
	\tadAxiomas [\paratodo{mapa}{\ m}, \paratodo{sistema}{\ s}, \paratodo{jugador}{\ j, j'}, \paratodo{posicion}{\ p, p'}, \paratodo{pokemon}{\ pk}, \paratodo{nat}{\ n}, \\ \paratodo{conj(jugador)}{\ js}, \paratodo{conj(posicion)}{\ ps}, \paratodo{multiconj(pokemon)}{\ pks}, \paratodo{conj(multiconj(pokemon))}{\ cpks}]
		...
		\tadAxioma{capturando?(j, ps, s)}{\IF vacio?(ps) THEN false ELSE enRangoDeCaptura?(j, dameUno(ps), s) $\lor$ capturando(j, sinUno(ps), s) FI}
