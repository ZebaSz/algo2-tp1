\begin{tad}{\tadNombre{Sistema}}
	...
	\tadUsa{\tadNombre{Nat},\tadNombre{Pokemon}, \tadNombre{Jugador}, \tadNombre{Mapa}, \tadNombre{Posicion}, \tadNombre{Bool}, \tadNombre{Conjunto($\alpha$)}, \tadNombre{Multiconjunto($\alpha$)}}
	...
	\tadGeneradores
		...
		%%Agrego cotizacion al principio
		...
		\tadOperacion{cambiarCotizacion}{sistema/s, nat/n}{}{0 $<$ n}
		%% TODO:Agregar que el precio de los pokemons del jugador pase la boleta y que el jugador pertenezca al juego
		\tadOperacion{pagarBoleta}{sistema/s, jugador/j, nat/n}{}{}
	...
	\tadOtrasOperaciones
		...
		\tadOperacion{valorPokemones}{conj(pokemon)/pks, sistema/s}{nat}{0 $< \#$todosLosPokemons(s)}
		...
		% TODO mandar esto y las otras operaciones a extensiones de TAD
	...
	\tadAxiomas [\paratodo{mapa}{\ m}, \paratodo{sistema}{\ s}, \paratodo{jugador}{\ j, j'}, \paratodo{posicion}{\ p, p'}, \paratodo{pokemon}{\ pk}, \paratodo{nat}{\ n, m}, \\ \paratodo{conj(jugador)}{\ js}, \paratodo{conj(posicion)}{\ ps}, \paratodo{multiconj(pokemon)}{\ pks}, \paratodo{conj(multiconj(pokemon))}{\ cpks}]
		...
		\tadAxioma{capturaPokemon?(j, ps, j', p, s)}
		{\IF vacio?(ps) THEN
			false
		ELSE
		...
		\tadAxioma{jugadoresDisponiblesACapturar(js, p, j, p', s)}
		{\IF vacio?(js) $\lor$ distancia(p, p') $\leq$ 2 THEN
			$\emptyset$
		ELSE
			{\IF movsLejosDePos(dameUno(js), s) = 9 $\land$ j $\neq$ dameUno(js) THEN
				Ag(dameUno(js), $\emptyset$)
			ELSE
				$\emptyset$
			FI} $\cup$ jugadoresDisponiblesACapturar(sinUno(js), p, j, p', s)
		FI}
		...
		\tadAxioma{agregarATodos(pk, cpks)}
		{\IF vacio?(cpks) THEN
			$\emptyset$
		ELSE
