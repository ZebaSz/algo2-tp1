\begin{tad}{\tadNombre{Sistema}}
	...
	\tadAxiomas
		...
		%TODO REVISAR EL CASO QUE EN EL MOVIMIENTO SE ELIMINE EL TIPO, HACER JUGADORES CONECTADOS
		\tadAxioma{pokemonsCapturados(j, moverJugador(s,j',p))}{pokemonsACapturar(j, posiciones(mapa(s)), moverJugador(s,j',p)) $\cup$ pokemonsCapturados(j,s)}

		\tadAxioma{pokemonsCapturados(j, agregarPokemon(s,pk,p))}{pokemonsCapturados(j,s)}



		\tadAxioma{pokemonsACapturar(j, ps, s)}
		{\IF vacio?(ps) THEN
			$\emptyset$
		ELSE
		...
		\tadAxioma{movimientosCercaDePos(j, p, registrarJugador(s,j'))}{\IF j = j' THEN 0 ELSE movimientosCercaDePos(j, p, s) FI}
		\tadAxioma{movimientosCercaDePos(j, p, conectarJugador(s,j',p'))}{\IF j = j' THEN 0 ELSE movimientosCercaDePos(j, p, s) FI}
		\tadAxioma{movimientosCercaDePos(j, p, desconectarJugador(s,j'))}{\IF j = j' THEN 0 ELSE movimientosCercaDePos(j, p, s) FI}
		\tadAxioma{movimientosCercaDePos(j, p, agregarPokemon(s,pk,p'))}{\IF p = p' THEN 0 ELSE movimientosCercaDePos(j, p, s) FI}

		\tadAxioma{movimientosCercaDePos(j, p, moverJugador(s,j',p'))}
		...
		%TODO verificar bien los casos de 0 para restricciones
