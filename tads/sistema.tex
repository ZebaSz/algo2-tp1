\begin{tad}{\tadNombre{Sistema}}
	...
	\tadOtrasOperaciones
		...
		\tadOperacion{filtrarPokemons}{pokemon, multiconj(pokemon)}{multiconj(pokemon)}{}
	...
	\tadAxiomas
		...
		\tadAxioma{rareza(pk, s)}{100 - 100 $\times$ $\frac{\#filtrarPokemons(pk,todosLosPokemons(s))}{\#todosLosPokemons(s))}$}
		...
		\tadAxioma{filtrarPokemons(pk, pks)}
		{\IF vacio?(pks) THEN
			$\emptyset$
		ELSE
			{\IF dameUno(pks) = pk THEN
				Ag(pk,filtrarPokemons(sinUno(pks)))
			ELSE
				filtrarPokemons(sinUno(pks))
			FI}
		FI}
