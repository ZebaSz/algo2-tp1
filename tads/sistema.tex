\begin{tad}{\tadNombre{Sistema}}
	...
	\tadAxiomas
		...
		{\IF conectado?(j) $\yluego$ capturaPokemon?(j, posiciones(mapa(s)), moverJugador(s,j',p)) THEN
			Ag(pokemonACapturar(j, posiciones(mapa(s)), moverJugador(s,j',p)), pokemonesCapturados(j, s))
		ELSE
			pokemonesCapturados(j, s)
		FI}
		...
		{\IF vacio?(ps - p) THEN
			false
		ELSE
			{\IF distancia(p, dameUno(ps - p)) $<$ 10 $\land$ hayPokemon?(dameUno(ps - p), s) THEN
				false
			ELSE
				pokemonsCerca?(p, Ag(p, sinUno(ps - p)), s)
			FI}
		FI}
