\begin{tad}{\tadNombre{Sistema}}
	...
	\tadObservadores
		...
		%%agrego la cotizacion como un observador
		\tadOperacion{pokeCotizacion}{sistema/s}{}{}
	...
	\tadAxiomas [\paratodo{mapa,}{\ m} \paratodo{sistema,}{\ s} \paratodo{jugador,}{\ j, j'} \paratodo{posicion,}{\ p, p'} \paratodo{pokemon,}{\ pk} \paratodo{nat,}{\ n, m} $ \\ $\paratodo{conj(jugador),}{\ js} \paratodo{conj(posicion),}{\ ps} \paratodo{multiconj(pokemon)}{\ pks}]
		...
		\tadAxioma{pokemonsCapturados(j, pagarBoleta(s,j',n))}{pokemonsCapturados(j, s) - dameUno(menores(filtrar(partes(pokemonsCapturados(j, s)), n, s), s))}
		...
		ELSE
			{\IF n $\leq$ valorPokemones(dameUno(cpks), s) THEN
				Ag(dameUno(cpks), filtrar(sinUno(cpks), n, s))
			ELSE
				filtrar(sinUno(cpks), n, s)
			FI}
		FI}

		\tadAxioma{menores(cpks, s)}
		{\IF vacio?(sinUno(cpks)) $\oluego$ valorPokemones(dameUno(cpks), s) $<$ valorPokemones(sinUno(menores(sinUno(cpks), s)), s)THEN
			Ag(dameUno(cpks), $\emptyset$)
		ELSE			
			{\IF valorPokemones(dameUno(cpks), s) $=$ valorPokemones(sinUno(menores(sinUno(cpks), s)), s) THEN
				Ag(dameUno(cpks), menores(sinUno(cpks), s)))
			ELSE
				menores(sinUno(cpks), s)
			FI}
		FI}
